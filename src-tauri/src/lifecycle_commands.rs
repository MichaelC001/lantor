...
pub(crate) async fn start_agent(agent_id: Uuid, state: State<'_, AppState>) -> CommandResult<()> {
    let previous_status: String = sqlx::query_scalar("select status from agents where id = $1")
        .bind(agent_id)
    ...
    let (redispatched_tasks, retried_manual_work, inbox_wake_available) =
        match dispatch_agent_restart_backlog(&state.pool, agent_id).await {
            Ok(result) => result,
            Err(err) => {
                restore_agent_status(&state.pool, agent_id, &previous_status).await?;
                return Err(err);
            }
        };

    let pending_start_after_backlog_result = sqlx::query_scalar(
        r#"
        select id
        from supervisor_commands
        where command_type = 'start_agent'
    ...
    .fetch_optional(&state.pool)
    .await;
    let pending_start_after_backlog: Option<Uuid> = match pending_start_after_backlog_result {
        Ok(row) => row,
        Err(err) => {
            restore_agent_status(&state.pool, agent_id, &previous_status).await?;
