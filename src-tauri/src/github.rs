...
fn normalized_github_login(login: &str) -> CommandResult<String> {
    ...
    if login.is_empty() {
        return Err("GitHub review login is required".to_owned());
    }
    if login.len() > 100
        || !login
            .chars()
    ...
    {
        return Err("GitHub review login contains unsupported characters".to_owned());
    }
    Ok(login.to_owned())
}
...
pub(crate) async fn load_github_binding(
    ...
    let row = sqlx::query(
        ...
        select
            ...
            account_login, review_login, review_queue_synced_at, created_at, updated_at
...
async fn load_review_links(
    pool: &SqlitePool,
    channel_id: Uuid,
    repository_id: &str,
) -> CommandResult<HashMap<i64, GithubReviewLink>> {
    let rows = sqlx::query(
        r#"
        select
...
async fn decorate_review_requests(
    pool: &SqlitePool,
    binding: &GithubRepositoryBinding,
    requests: Vec<GithubPullRequestCli>,
) -> CommandResult<Vec<GithubPullRequest>> {
    let mut links = load_review_links(pool, binding.channel_id, &binding.repository_id).await?;
    Ok(requests
        .into_iter()
        .map(|pull_request| {
...
pub(crate) async fn refresh_github_channel_overview(
    ...
    let requests = search_review_requests(&binding).await?;
...
mod tests {
    use super::{
        create_github_review_task_record, load_cached_github_channel_overview,
        parse_review_requests, replace_cached_review_requests, GithubActorCli,
        GithubPullRequestCli, GithubPullRequestDetail, GithubRepositoryBinding,
    };
    ...
    async fn cached_review_queue_replaces_stale_snapshot() -> Result<(), String> {
        ...
        let result = async {
            ...
            let second_snapshot = vec![GithubPullRequestCli {
                number: 42,
