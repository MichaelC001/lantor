use super::{
    dispatch_agent_restart_backlog, mark_task_after_work_item_finished, try_claim_unassigned_task,
};
...
async fn restart_backlog_retries_failed_explicit_manual_work() {
    ...
    let result: Result<(), String> = async {
        ...
        let inbox = sqlx::query("select work_item_id, state from agent_inbox_items where id = $1")
            .bind(inbox_item_id)
            .fetch_one(&pool)
            .await
            .map_err(|err| err.to_string())?;
...
async fn merged_task_assigned_work_item_marks_every_task_ready_for_review() {
    ...
    let result: Result<(), String> = async {
        ...
        mark_task_after_work_item_finished(&pool, work_item_id, agent_id, run_id, "done").await?;
